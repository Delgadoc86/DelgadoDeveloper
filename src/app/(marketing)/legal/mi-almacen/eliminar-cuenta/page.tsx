...
// `features` de Mi Almacén en projects.ts) y hay que construirlo antes de publicar.
// Antes de activarla: confirmar plazos reales, agregar el flujo in-app, y sacar
// `robots: { index: false }` de abajo.
export const metadata: Metadata = {
  title: "Cómo eliminar tu cuenta y tus datos — Mi Almacén",
  description:
    "Cómo pedir la eliminación de tu cuenta de Mi Almacén y de los datos asociados.",
  alternates: {
    canonical: "/legal/mi-almacen/eliminar-cuenta",
  },
  robots: { index: false, follow: false },
};

export default function MiAlmacenEliminarCuentaPage() {
  return (
          ...
          <div className="text-foreground-muted mt-10 space-y-8 text-sm leading-relaxed">
            <div>
              ...
              <p className="mt-2">
                ...
                la app, en Configuración. Google Play la exige además de este medio web.]
              </p>
              <p className="mt-2">
                Mientras esa opción no esté disponible en la app, podés pedir la
                eliminación de tu cuenta y tus datos escribiendo a{" "}
                <a
                  href={`mailto:${siteConfig.author.email}?subject=${encodeURIComponent("Eliminar mi cuenta de Mi Almacén")}`}
            ...
            <div>
              ...
              <p className="mt-2">
                Tu cuenta (Firebase Authentication) y los datos asociados en Firestore:
                datos del comercio, productos, categorías, clientes fiados y movimientos
                cargados con esa cuenta.
              </p>
              <p className="mt-2">
