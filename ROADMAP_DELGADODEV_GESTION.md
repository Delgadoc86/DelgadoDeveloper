...
| 2026-08-06 | 3     | Post-launch: rebranding público PresuFácil → PresuPDF, pedido por Cristian para dejar de indexar "presufacil" y evitar confusión con el dominio ya existente `presufacil.com`. Nueva URL pública `/proyectos/presupdf` y `/descargar/presupdf`; las rutas viejas (`/proyectos/presufacil`, `/descargar/presufacil`) quedan solo como redirect 308 permanente en `next.config.ts` — no vuelven a renderizar contenido. `descargar/[slug]/route.ts` suma un alias interno (`presupdf` → `apps/presufacil`) para que el documento de Firestore, sus datos y el enlace real del APK **no se toquen**. `projects.ts` suma `analyticsId: "presufacil"` para que el evento de Analytics `download_click_presufacil` (ver README) no pierda continuidad histórica pese al cambio de slug/URL público. Título, meta description, Open Graph, Twitter Card, JSON-LD, canonical, sitemap, H1 y `alt` de imágenes pasan a "PresuPDF"; se agregó "Antes PresuFácil" visible junto al H1 y en la descripción, a retirar en ~6 meses o cuando el tráfico por "PresuFácil" en Search Console deje de aportar. Las 4 páginas legales (`/legal/presufacil/*`) **no se tocaron** — quedan pendientes de una decisión aparte sobre migrarlas a `/legal/presupdf/*` | `npm run build` + `next start` local contra Firestore real: `/proyectos/presufacil` y `/descargar/presufacil` → 308 confirmado con curl; `/descargar/presupdf` resolvió el doc real `apps/presufacil` y redirigió 302 al APK real en Drive; sitemap solo lista `presupdf`; canonical/OG/Twitter/JSON-LD apuntan a la URL nueva; `/admin` y `/proyectos/mi-almacen` responden igual que antes (sin regresión); Firestore, reglas, credenciales y panel admin sin cambios | Claude            |
...
| Rutas legales `/legal/presufacil/*`    | Mantener tal cual / migrar a `/legal/presupdf/*` con redirect | Sin decidir — Cristian debe confirmar antes de tocar las 4 páginas (son texto legal, no solo SEO) | 2026-08-06 |
